...
/**
 * How a job scores against the signed-in seeker's profile.
 *
 ...
 * thing.
 *
 * It explains an order; it does not set one. The board is sorted newest-first in
 * Mongo, and a score computed after the query could only rank the rows already
 * on the page.
 *
 * `fit` is absent for an anonymous visitor and for a recruiter — both reach the
 * same public routes — and absent renders nothing rather than 0%.
 */
export function FitBadge({ fit, className }: { fit?: ScoreBreakdown; className?: string }) {
  if (!fit) return null;
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-x-2 gap-y-1", className)}>
      ...
      */}
      <Badge variant="outline" className="border-signal text-signal-text">
        {points(fit.score)}% fit
      </Badge>
      <span className="text-xs text-ink-muted">
        {worst ? worst.reason : "Matches everything you asked for"}
      </span>
    </div>
  );
}
...
export function FitBreakdown({ fit, className }: { fit?: ScoreBreakdown; className?: string }) {
  ...
  return (
    <section
      ...
      className={cn(
        "rounded-surface border border-line bg-paper-sunken p-(--space-card)",
        className,
      )}
    >
